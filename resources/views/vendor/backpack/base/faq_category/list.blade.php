                              ...
                              @foreach ($data as $content)
                                <tr data-element-id='{{ $content->id }}' data-sort='{{ $content->sort }}'>
                                   ...
                                   <!--  <td>
                                    ...
                                    <td><?php if($content->status == 0){ ?><a href="{{ url(config('backpack.base.route_prefix', 'admin').'/faq_category/status/'.$content->id.'/1') }}"><span class="badge bg-red">Inactive</span></a><?php  }else{ ?><a href="{{ url(config('backpack.base.route_prefix', 'admin').'/faq_category/status/'.$content->id.'/0') }}"><span class="badge bg-green">Active</span></a><?php } ?></td>
    ...
    $('.sortable-treatment').sortable({
        ...
        stop: function( event, ui ) {
            ...
            $('#element-order > tr').each(function(){
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ url(config('backpack.base.route_prefix').'/faq_category/update_sort') }}",
